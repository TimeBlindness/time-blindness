...
def analyze_results(results_file, metadata_csv, output_dir):
    ...
    for category in categories:
        ...
        logger.info(
            f"Category '{category}' accuracy: {accuracy:.4f} ({correct_count}/{len(results)})"
        )
...
def visualize_results(results, output_path):
    ...
    plt.axhline(
        y=results["overall_accuracy"],
        color="r",
        linestyle="-",
        label=f'Overall: {results["overall_accuracy"]:.4f}',
    )

    plt.title("Qwen VL Performance on SpookyBench")
    plt.xlabel("Category")
    plt.ylabel("Accuracy")
    plt.ylim(0, 1.0)
    plt.legend()
...
def main():
    parser = argparse.ArgumentParser(description="Analyze Qwen VL SpookyBench results")
    parser.add_argument(
        "--results", type=str, required=True, help="Path to JSON results file"
    )
    parser.add_argument(
        "--csv", type=str, required=True, help="Path to SpookyBench metadata CSV"
    )
    parser.add_argument(
        "--output",
        type=str,
